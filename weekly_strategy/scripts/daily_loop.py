...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--time-utc", default=DEFAULT_TIME_UTC,
        help="Time of day to fire, HH:MM UTC. Default %(default)s "
             "(5:30 PM ET, after US close).",
    )
    parser.add_argument(
    ...
    hh, mm = _parse_hhmm(args.time_utc)
    ...
    logger.info(
        "daily loop started; time=%02d:%02d UTC, mode=%s, weekdays_only=%s, pid=%d",
        hh, mm, args.mode, args.weekdays_only, os.getpid(),
    )

    while True:
        now = datetime.now(timezone.utc)
        target = _next_run_time(now, hh, mm)
        wait_s = (target - now).total_seconds()
        logger.info(
            "next run at %s UTC  (sleep %.0fs / %.1fh)",
            target.isoformat(timespec="seconds"), wait_s, wait_s / 3600,
        )
        try:
            time.sleep(wait_s)
        ...
        if args.weekdays_only and target.weekday() >= 5:
            logger.info("weekend (dow=%d); skipping", target.weekday())
            continue
